...
legend_ord_fuels <- c("BEV", "Electricity", "Hybrid Electric", "PHEV", "FCEV", "Hydrogen", "Hybrid Liquids", "Hybrids", "Liquids", "Oil", "Biodiesel", "Synfuel", "Synfuels", "Gases","NG", "Coal", "Gas", "Geothermal", "Uranium", "Bioliquids", "Hydro", "Solar", "Wind")

legend_ord_costs <- c("Inconvenience", "Risk", "Charger", "Model availability", "Range anxiety", "Refuel availability", "Policy-induced inconv.","Fuel", "Purchase", "O&M")
...
theme2use <- theme_minimal()+
  theme(axis.text.x = element_text(angle = 90, size=14, vjust=0.5, hjust=1),
          ...
          panel.grid =   element_line(colour = "grey",size=0.45))
...
theme2useJournal <- theme_minimal()+
...
kayapf = function(dtES, dtFE, dtEmi, rp, mapping){
    dtES = dtES[region == rp & mode == "pass" & year >= 2015 & vehicle_type_plot %in% c("Small Cars","Large Cars")]
    dtES = dtES[,.(demand_F = sum(demand_F)), by = .(region, year, scenario)]
    dtFE = dtFE[region == rp & year >= 2015]
    dtFE = dtFE[aggr_mode %in% c("LDV")]
    dtFE = dtFE[,.(demand_EJ = sum(demand_EJ)), by = .(region, year, scenario)]

    dtEmi = dtEmi[region == rp & year >= 2015 & variable %in% c("Emi|CO2|Transport|Pass|Road|LDV|Demand"),]
    ...
    p = ggplot()+
      geom_line(data = dt[year <= 2050], aes(x = year, y = value, group = scen_name, color = scen_name), size = 0.8)+
...
## indexed to 2015
ggsave("FIG12.jpg", p, dpi=1000, height = width* aspect_ratio , width = width, units = "cm")
...
costspf = function(dt, rp, mapping_scens){
    ...
    map = data.table(name = c("Risk", "Charger", "Model availability", "Range anxiety", "Refuel availability", "Fuel", "Policy-induced inconv.", "Purchase", "O&M"),
    ...
    plot_hist = ggplot()+
      geom_bar(data = dt_tot[year == 2020 & scen_name == "NPi_Conv"][,scen_name := "Current (est.)"], aes(x = technology, y = cost, group = name, fill = name, alpha = alph), position = "stack", stat = "identity")+
    ...
    plot_hist = ggplot()+
      geom_bar(data = dt_inc[year == 2020 & scen_name == "NPi_Conv"][,scen_name := "Current (est.)"], aes(x = technology, y = cost, group = name, fill = name), position = "stack", stat = "identity")+
...
ggsave("FIG7.jpg", p, dpi=1000, height = width* aspect_ratio , width = width, units = "cm")
```
## Costs for 2015
...
costs_detailed_pf = function(dt, rp, mapping_scens){
    ...
    dt_tot[, maxval := max(sum)]
    dt_tot[, name := factor(name, levels = c("Risk", "Charger", "Model availability", "Range anxiety", "Refuel availability", "Fuel", "Purchase", "O&M"))]
...
ggsave("FIG4.jpg", p, dpi=1000, height = width* aspect_ratio , width = width, units = "cm")
...
ggsave("FIG6.jpg", p, dpi=1000, height = width* aspect_ratio , width = width, units = "cm")
```
...
aspect_ratio <- 0.5
width <- 14
ggsave("FIGC1.jpg", p, dpi=1000, height = width* aspect_ratio , width = width, units = "cm")
```
...
LDV_PEnewpf = function(dt, rp, mapping){
   ...
   dt[technology == "Biomass", technology := "Bioliquids"]
...
aspect_ratio <- 0.3
width <- 19
ggsave("FIG10.jpg", p, dpi=1000, height = width* aspect_ratio , width = width, units = "cm")
```
...
aspect_ratio <- 0.3
width <- 19
ggsave("Fig11_alltr.jpg", p, dpi=1000, height = width* aspect_ratio , width = width, units = "cm")
```
## PE in HDVs by fuel
...
theme(strip.text.y = element_text(size = 8),
 ...
 dtboh = dtboh[,.(value = sum(value)), by = .(scen_name, year)]
 dtboh = dtboh[year>=2020]
 plot2 = ggplot()+
    geom_line(data = dtboh, aes(x = year, y = value, group = scen_name, color = scen_name), size=1)+
    scale_x_continuous(breaks = c(2020, 2030, 2050))+
    theme2useJournal +
    labs(x = "", y = expression(paste("[Mt", CO["2"], "]")))+
    guides(shape = FALSE) +
    scale_color_manual("Scenario", values = cols) +
    theme(legend.position = "right")+
    ylim(c(0,600))
...
aspect_ratio <- 0.3
width <- 19
ggsave("Fig13_alltversion.jpg", p$plot1,  dpi=1000,  height = width* aspect_ratio , width = width, units = "cm")

aspect_ratio <- 0.7
width <- 9

ggsave("FIG11.jpg", p$plot2, dpi = 1000, height = width* aspect_ratio , width = width, units = "cm")
```
...
EJmodepf = function(dt, rp,  mapping){
  ...
  plot = ggplot()+
    ...
    labs(x = "", y = "[EJ]")+
    facet_wrap(~scen_name, nrow = 1)+
    scale_fill_manual("Mode", values = cols, breaks = legend_ord)+
    expand_limits(y = c(0,1))+
...
aspect_ratio <- 0.3
width <- 19
ggsave("FIG10_allt.jpg", p, dpi=1000, height = width* aspect_ratio , width = width, units = "cm")
...
EJfuelsMode_pf = function(dt, rp, mapping){
  dt = dt[year >= 2020 & year <= 2050 & region == rp]
  dt = merge(dt, mapping, by = "scenario")
  dt[subtech == "Biomass", subtech := "Bioliquids"]
  dt[, subtech := factor(subtech, levels = c("Electricity", "Hydrogen", "NG", "Oil", "Bioliquids", "Synfuel"))]
  dt[, scen_name := factor(scen_name, levels = c("NPi_Conv", "B1100_Conv", "B1100_ConvSyn", "B1100_H2Push", "B1100_ElecPush", "B1100_Conv-LowD", "B1100_ElecPush-LowD"))]
  legend_ord_FE = c("Electricity", "Hydrogen", "NG", "Oil", "Bioliquids", "Synfuel")
  plotLDV = ggplot()+
    ...
    labs(x = "", y = "[EJ]")+
    facet_wrap(~scen_name, nrow = 1)+
    scale_fill_manual("Technology",values = cols, breaks=legend_ord_FE)+
    expand_limits(y = c(0,1))+
    scale_x_continuous(breaks = c(2020, 2030, 2050)) +
    theme2useJournal +
    guides(fill=guide_legend(nrow=1,byrow=TRUE))+
...
aspect_ratio <- 0.3
width <- 19
ggsave("FIG9.jpg", pLDV, dpi=1000, height = width* aspect_ratio , width = width, units = "cm")
ggsave("FEfuelModeFreight.jpg", pfre, dpi=500, height = width* aspect_ratio , width = width, units = "cm")
ggsave("FEfuelModeAllT.jpg", pAllT, dpi=500, height = width* aspect_ratio , width = width, units = "cm")
```
...
aspect_ratio <- 0.3
width <- 19
ggsave("FIG8.jpg", ppass, dpi=1000, height = width* aspect_ratio , width = width, units = "cm")
ggsave("ESModeFr.jpg", pfre, dpi=500, height = width* aspect_ratio , width = width, units = "cm")
```
## Emissions by sector
